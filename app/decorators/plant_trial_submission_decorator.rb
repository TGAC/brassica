class PlantTrialSubmissionDecorator < SubmissionDecorator
  ...
  def trait_descriptors
    ...
    tds = non_numeric_tds | TraitDescriptor.where(id: numeric_tds).pluck(:descriptor_name)
    tds.map { |td| h.html_escape td }
  end
  ...
  def sorted_trait_names
    ...
    @sorted_trait_names = trait_list.compact.map do |trait_item|
      if trait_item.to_i == 0
        h.html_escape trait_item
      else
        trait = TraitDescriptor.where(id: trait_item).first
        trait ? h.html_escape(trait.descriptor_name) : nil
      end
    end
  end
