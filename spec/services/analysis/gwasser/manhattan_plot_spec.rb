...
RSpec.describe Analysis::Gwasser::ManhattanPlot do
  ...
  context "with no map file available" do
    let(:expected_trait_results) {
      [
        ["snp.1", 42.248475811326],
        ["snp.2", 2.71525554601752],
        ["snp.3", 197.525150438166]
      ]
    }
    ...
    it "returns mutations per trait" do
      expect(subject[:traits].map { |trait| trait[0] }).to match_array(%w(trait_5 trait_6 trait_7))
  ...
  context "with map file available" do
    ...
    let(:expected_trait_results) {
      [
        ["snp-3", 197.525150438166, "1", 3],
        ["snp-1", 42.248475811326, "1", 14],
        ["snp-2", 2.71525554601752, "2", 4]
      ]
    }
