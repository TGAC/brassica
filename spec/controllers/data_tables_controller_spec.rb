  ...
  context 'when unauthenticated' do
    describe '#index' do
      ...
      it 'prevents getting model that is not permitted' do
        expect{ get(:index, params: { model: 'unpermitted_models' }) }.
          to raise_error ActionController::RoutingError
        expect{ get(:index, format: :json, params: { model: 'unpermitted_models' }) }.
          to raise_error ActionController::RoutingError
      end

      it 'returns table template on html format request' do
        DataTablesController.new.send('allowed_models').each do |model|
          get :index, params: { model: model }
      ...
      it 'does not raise error on wrong parameter json format request' do
        get :index, format: :json, params: { model: 'plant_lines', query: { plant_line_name: 'wrong!' } }
        expect(response).to have_http_status(:success)
      end

      it 'does not render htmls on json format request' do
        get :index, format: :json, params: { model: 'plant_populations' }
      ...
      it 'supports query filtering on json format request' do
        ...
        get :index, format: :json, params: { model: 'plant_populations', query: { id: pps[0] } }
        expect(response.content_type).to eq 'application/json'
        json = JSON.parse(response.body)
      ...
      it 'avoids double joins when querying qtl by publishable parameters' do
        ...
        get :index, format: :json, params: { model: 'qtl', query: { 'linkage_groups.id' => lg.id } }
        expect(response.content_type).to eq 'application/json'
        json = JSON.parse(response.body)
        expect(json['recordsTotal']).to eq 1
        expect(json['data'].size).to eq 1
      ...
      it 'avoids double joins when querying trait_scores by publishable parameters' do
        ...
        _ts1 = create(:trait_score)
        ts2 = create(:trait_score, trait_descriptor: td)
        get :index, format: :json, params: { model: 'trait_scores', query: { 'trait_descriptors.id' => td.id } }
        expect(response.content_type).to eq 'application/json'
        json = JSON.parse(response.body)
        expect(json['recordsTotal']).to eq 1
        expect(json['data'].size).to eq 1
      ...
      it 'avoids double joins when querying marker_assays by publishable parameters' do
        ...
        _ma1 = create(:marker_assay)
        ma2 = create(:marker_assay, primer_a: ps[0])
        get :index, format: :json, params: { model: 'marker_assays', query: { primer_a_id: ps[0].id } }
        expect(response.content_type).to eq 'application/json'
        json = JSON.parse(response.body)
        expect(json['recordsTotal']).to eq 1
        expect(json['data'].size).to eq 1
      ...
      it 'prevents querying by unpermitted parameters' do
        ...
        get :index, format: :json, params: { model: 'plant_lines', query: { named_by_whom: 'nbw' } }
        json = JSON.parse(response.body)
    ...
    describe '#show' do
      it 'escapes annotation values' do
        ...
        get :show, format: :json, params: { id: pp.id, model: 'plant_populations' }
  ...
  context 'when authenticated' do
    ...
    describe '#index' do
      it 'returns datatables json on json format request' do
        ...
        _pp3 = create(:plant_population, published: false, user: u2)
